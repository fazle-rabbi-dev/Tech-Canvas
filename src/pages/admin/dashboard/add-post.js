import { useAddPost, useImageUpload } from "@/hooks";
...
function AddPost() {
  const { title,thumbnail, desc, tags, date, content, onChange,handleSubmit,loading } = useAddPost();
  const {
    imageField,
    handleFileUpload,
    imageDownloadURL,
  ...
  return (
    <section className="mb-20">
      ...
      >
        ...
        {/* Image Upload */}
        <div className="rounded mb-2 p-2 bg-white dark:bg-gray-800 border-[1px] ring-1 ring-gray-300 dark:ring-0">
          <div className="w-full">
            {
              authToken === "guest" && (
                  <span className="text-sm italic font-bold leading-5 mb-2 block">Since, you are a <span className="text-purple-600 dark:text-purple-300">guest</span> user so, you can't upload any files!</span>
                )
            }
            <input
              ...
              className="border-0 w-full rounded outline-0"
              type="file"
              id="image"
          ...
          <div className="my-2 relative">
            ...
            <div className={`${imageDownloadURL ? "visible" : "hidden"}`}>
              <span className="mt-2 inline-flex gap-2 text-gray-600 text-xl font-bold">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
        ...
        <button
          disabled={loading}
          className="flex justify-center items-center font-satoshi-medium bg-purple-600 text-white px-4 py-[7px] w-full text-center rounded disabled:bg-purple-500"
          type="submit"
        >
          Post 
        </button>
